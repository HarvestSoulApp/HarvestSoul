        ...
        .then((event) => {
            ...
            User.find({_id:{$in:interested}}).then((users) => {
                ...
                res.render('event/event', {
                    date, description, location, _id, currentUserId, usernames, organizerName, imageUrl,
                    comparisonResult
                })
            })
        })
...
router.post('/:eventId/delete', isLoggedIn, (req, res, next) => {
    const eventId = req.params.eventId
    Event.findOneAndDelete({ _id: eventId, organizer: req.session.currentUser._id })
        .then((event) => {
            if (!event) {
                res.redirect('/auth/unauthorized')
            }
            res.redirect('/event/find')
        })
        .catch(error => {
